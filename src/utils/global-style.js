...
export const convertToResponsiveStyle = (body) => {
	let currentCss = '';
    let css = [];
	let coreCss = [];
    loadPromise.then(() => {
		...
		settings.fetch().then((response) => {
			...
			for (let i = 0; i < colors.length; i++) {
				...
				let colorTypographyName = 'body .has-' + colorName.split(' ').join('-') + '-color';
				let colorBackgroundName = 'body .has-' + colorName.split(' ').join('-') + '-background-color';

				// Only do this for our global colors.
				if ( color['color'] && color['slug'] ) {
					// Add the custom css property.
					css.push('--' + color['slug'] + ': ' + color['color'] + ';');
					css.push('--' + color['slug'] + '-rgba: ' + color['rgb'] + ';');

					// Add custom css class rule for other blocks.
					// For typography colors.
					coreCss.push(colorTypographyName + ' { color: ' + color['color'] + ' !important; }');

					// For background colors.
					coreCss.push(colorBackgroundName + ' { background-color: ' + color['color'] + ' !important; }');
				}
			}
			...
			for (const selector in typography) {
				...
				for (const attrName in attrs) {
					if ( ! attrName.includes('Tablet') && ! attrName.includes('Mobile') ) {
				...
				typoCSS.push(typoCSSSelector + JSToCSS(typoAttrs, attrs) + '}')
				typoCSSTablet.push(typoCSSSelector + JSToCSS(typoAttrsTablet, attrs) + '}')
				typoCSSMobile.push(typoCSSSelector + JSToCSS(typoAttrsMobile, attrs) + '}')
			}
			...
			if ( 0 < typoCSSTablet.length ) {
				currentCss += '\n@media(max-width:'+wcb_params.breakpoints['tablet']+'px) {';
				currentCss += typoCSSTablet.join(' ');
				currentCss += '}';
			}
			if ( 0 < typoCSSMobile.length ) {
				currentCss += '\n@media(max-width:'+wcb_params.breakpoints['mobile']+'px) {';
...
export const renderGlobalStyle = () => {
	setTimeout(() => {
		const _renderGlobalStyle = (body) => {
			if (body) {
				convertToResponsiveStyle(body);
			}
		};
		const bodyElement = getDocumentBody();
		_renderGlobalStyle(bodyElement);
		if (bodyElement !== document.querySelector('body')) {
			_renderGlobalStyle(document.querySelector('body'));
		}
	}, 50);
};
...
export const removeOldStyleTag = (body = document.querySelector('body')) => {
