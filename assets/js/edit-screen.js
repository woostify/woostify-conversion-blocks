...
var wcbRegenerateCssFiles = function () {
	...
	button.onclick = function () {
		...
		var request = new Request( wcb_edit_screen.ajax_url, {
			method: 'POST',
			body:
				'action=regenerate_css_files&wcb_settings_form_nonce=' +
				wcb_edit_screen.settings_form_nonce,
...
var woostifyMultiSelect = function () {
	...
	meta.forEach( function ( element ) {
		var input = element.querySelector( '.woostify-multi-select-value' ),
			selection = element.querySelector(
				'.woostify-multi-select-selection'
			),
			inner = selection.querySelector(
				'.woostify-multi-selection-inner'
			),
			search = selection.querySelector( '.woostify-multi-select-search' ),
			dropdown = element.querySelector(
				'.woostify-multi-select-dropdown'
			);

		// Save value.
		var saveItem = function ( echo ) {
			var selectionItem = inner.querySelectorAll(
				'.woostify-multi-select-id'
			);
			...
			var data = [];
			selectionItem.forEach( function ( ele ) {
		...
		var removeItem = function () {
			...
			selectionItem.forEach( function ( el ) {
				...
				removeButton.onclick = function () {
					...
					var isThis = dropdown.querySelector(
						'[data-id="' + selectedId + '"]'
					);
					if ( isThis ) {
		...
		var addItem = function () {
			var dropdownItem = dropdown.querySelectorAll(
				'.woostify-multi-select-id'
			);
			...
			for ( var i = 0, j = dropdownItem.length; i < j; i++ ) {
				dropdownItem[ i ].onclick = function () {
					...
					var currentId =
						'<span class="woostify-multi-select-id" data-id="' +
						dataId +
						'">' +
						t.innerHTML +
						'<i class="woostify-multi-remove-id dashicons dashicons-no-alt"></i></span>';
							...
							.then( function ( res ) {
								...
								res.json().then( function ( r ) {
									...
									var parser = new DOMParser(),
										...
										ajaxDropdown = doc.querySelectorAll(
											'.woostify-multi-select-id'
										);

									if ( ajaxDropdown.length ) {
										ajaxDropdown.forEach( function (
											ajaxItem
										) {
											var ajaxItemId =
													ajaxItem.getAttribute(
														'data-id'
													) || '',
												saveValue =
													saveItem( true ) || [];
											...
											if ( saveValue.length ) {
												if (
													...
													ajaxItem.classList.add(
														'disabled'
													);
												} else {
													ajaxItem.classList.remove(
														'disabled'
													);
												}
											}
		...
		document.addEventListener( 'click', function ( e ) {
			var current = e.target,
				...
				isRemove = current.classList.contains(
					'woostify-multi-remove-id'
				),
				isSelect = current.classList.contains(
					'woostify-multi-select-id'
				),
				isSearch = current.classList.contains(
					'woostify-multi-select-search'
				);
...
var woostifyMultiDependency = function () {
	...
	dep.forEach( function ( element ) {
		var item = element.querySelectorAll(
			'.woostify-multi-dependency-item [name]'
		);
		if ( ! item.length ) {
			return;
		}
		...
		item.forEach( function ( ele, index ) {
			...
			ele.addEventListener( 'change', function () {
				var currentValue = ele.value,
					...
					allDependency = element.querySelectorAll(
						'.woostify-multi-dependency-item:nth-child(' +
							currentChild +
							') ~ .woostify-multi-dependency-item[data-required="' +
							currentValue +
							'"]'
					),
				...
				if ( allDependency.length ) {
					allDependency.forEach( function ( el ) {
...
var woostifyTabSettings = function () {
	...
	button.forEach( function ( element ) {
		element.onclick = function () {
			var id = element.hash ? element.hash.substr( 1 ) : '',
				...
				tab = section.querySelector(
					'.woostify-setting-tab-content[data-tab="' + id + '"]'
				),
			...
			if ( idSiblings.length ) {
				idSiblings.forEach( function ( el ) {
					el.classList.remove( 'active' );
				} );
			}
	...
	window.addEventListener( 'load', function () {
		var currentTab = section.querySelector(
...
var woostifySaveOptions = function () {
	...
	buttons.forEach( function ( button ) {
		button.onclick = function () {
			if (
				button.getAttribute( 'disabled' ) ||
				box.classList.contains( 'loading' )
			) {
				return;
			}
			...
			if (
				box.classList.contains( 'woostify-ajax-search-product-setting' )
			) {
				var mustChooseOption = box.querySelector(
						'.must-choose-one-option'
					),
					checkboxInput = mustChooseOption
						? mustChooseOption.querySelector( '[type="checkbox"]' )
						: false,
					checkedOption = mustChooseOption
						? mustChooseOption.querySelectorAll(
			...
			if ( required.length ) {
				required.forEach( function ( field ) {
					var fieldValue = field.value.trim(),
						fieldParent = field.closest(
							'.woostify-setting-tab-content'
						),
						isHidden = field.closest(
							'.woostify-filter-item.hidden'
						),
						fieldInTab = fieldParent
							? fieldParent.getAttribute( 'data-tab' )
							: '';
					...
					if ( ! fieldValue ) {
						...
						var emptyTab = box.querySelector(
							'a[href="#' + fieldInTab + '"]'
						);
						if (
							emptyTab &&
							! emptyTab.classList.contains( 'active' )
						) {
							emptyTab.click();
						}
			...
			// Request.
			var request = new Request( ajaxurl, {
				method: 'POST',
				body:
					'action=save_' +
					likeId +
					'_options&security_nonce=' +
					_nonce +
					'&setting_id=' +
					_id +
					'&options=' +
			...
			fetch( request )
				.then( function ( res ) {
					...
					let infoSuccess = box.querySelector(
						'.woostify-settings-section-footer .info-success'
					);
					if ( ! infoSuccess ) {
						button.insertAdjacentHTML(
							'afterend',
							'<span class="info-success">' +
								wcb_edit_screen.saved_success +
								'</span>'
						);
					}
...
var woostifySaleNotificationRemoveMessage = function () {
	var messages = document.querySelector(
		'.woostify-sale-notification-box-message'
	);
	...
	var removeMessageButton = messages.querySelectorAll(
		'.woostify-sale-notification-remove-message'
	);
	if ( ! removeMessageButton.length ) {
		return;
	}

	removeMessageButton.forEach( function ( element ) {
		element.onclick = function () {
			var t = this,
	...
	document.documentElement.addEventListener( 'lastMessage', function () {
		var lastMessage = messages.querySelector(
			'.woostify-sale-notification-remove-message'
		);
		if ( ! lastMessage ) {
			return;
...
var woostifySaleNotificationAddNewMessage = function () {
	var box = document.querySelector( '.woostify-featured-setting' ),
		...
		firstMessage = messages
			? messages.querySelector(
					'.woostify-sale-notification-message-inner'
			  )
			: '',
		removeLabel = firstMessage
	...
	markupMessage +=
		'<textarea name="woostify_sale_notification_message[]" required="required"></textarea>';
	markupMessage +=
		'<span class="woostify-sale-notification-remove-message button">' +
		removeLabel +
		'</span>';
	markupMessage += '</div>';
	...
	var addNewButton = box.querySelector(
		'.woostify-sale-notification-add-message'
	);
	if ( ! addNewButton ) {
		return;
	}
	...
	addNewButton.onclick = function () {
		...
		var lastMessage = messages.querySelector(
				'.woostify-sale-notification-message-inner:last-of-type'
			),
			lastField = lastMessage
				? lastMessage.querySelector( 'textarea' )
				: false;
		...
		var removeDisabledAttr = messages.querySelector(
			'.woostify-sale-notification-remove-message[disabled="disabled"]'
		);
...
var woostifyHeaderFooterBuilderSelectTemplate = function () {
	var templateEl = document.getElementById(
		'woostify-header-footer-builder-template'
	);
	...
	if ( templateEl ) {
		templateEl.addEventListener( 'change', function () {
			...
			if ( 'default' === template ) {
				...
				var nonce = document.getElementById( 'woostify-hfb-nonce' )
					.value;
				if ( check.length === 0 ) {
					// Request.
					var request = new Request( ajaxurl, {
						method: 'POST',
						body:
							'action=woostify_pro_select_template&_ajax_nonce=' +
							nonce +
							'&template=' +
							template,
						credentials: 'same-origin',
						...
						res.json().then( function ( r ) {
							...
							if ( '' !== r.data ) {
								var parser = new DOMParser(),
									...
									ajaxInputWrapper = data.querySelector(
										'.input-wrapper'
									);
								document
									.querySelector(
										'.woostify-pro-hfb-options-wrapper'
									)
									.append( ajaxInputWrapper );
							}
						} );
					} );
				}
				var sticky = document.querySelector(
					'.condition-group.sticky'
				);
				if ( sticky !== null ) {
					if ( 'header' === template ) {
...
var woostifyChangeDisplayOption = function () {
	if ( document.getElementById( 'woostify-hfb-nonce' ) ) {
		...
		document.addEventListener( 'change', function ( e ) {
			...
			if (
				'woostify-header-footer-builder-display-on' === curr_id ||
				'woostify-header-footer-builder-no-display-on' === curr_id
			) {
				...
				var request = new Request( ajaxurl, {
					method: 'POST',
					body:
						'action=woostify_pro_load_autocomplete&_ajax_nonce=' +
						nonce +
						'&post_type=' +
						postType +
						'&type=' +
				...
				fetch( request ).then( function ( res ) {
					...
					res.json().then( function ( r ) {
						...
						conditionGroup.getElementsByClassName(
							'child-item'
						)[ 0 ].innerHTML = r.data;
					} );
				} );
		...
		document.addEventListener( 'click', function ( e ) {
			...
			if (
				curr.classList.contains( 'woostify-select-all-post' ) ||
				curr.classList.contains( 'woostify-arrow' ) ||
				curr.classList.contains( 'woostify-select-all' )
			) {
				...
				for ( var i = 0; i < childItemsCount; i++ ) {
					if (
						parent.children[ i ].classList.contains(
							'woostify-section-render--post'
						)
					) {
				...
				if ( render.classList.contains( 'hidden' ) ) {
					...
					parent
						.getElementsByClassName( 'woostify--hf-post-name' )[ 0 ]
						.focus();
				} else {
			...
			if (
				curr.classList.contains( 'post-item' ) &&
				! curr.classList.contains( 'disabled' )
			) {
				var inputItemWrapper = curr.closest( '.input-item-wrapper' ),
					listPost = inputItemWrapper.querySelector(
						'.woostify-post-ids'
					).value,
					all = inputItemWrapper.querySelector(
						'.woostify-select-all-post'
					),
					parentSelectPost = inputItemWrapper.querySelector(
						'.woostify-section-select-post'
					);
				...
				if ( ! listPost.includes( id ) ) {
					...
					var html =
						...
						title +
						'</span>' +
						'<span class="btn-woostify-auto-complete-delete ion-close" data-item="' +
						id +
						'"></span>';
					...
					inputItemWrapper
						.querySelector( '.woostify-auto-complete-field' )
						.insertBefore(
							newTagItem,
							inputItemWrapper.querySelector(
								'.woostify--hf-post-name'
							)
						);
				}
				inputItemWrapper.querySelector(
			...
			if (
				curr.classList.contains( 'btn-woostify-auto-complete-delete' )
			) {
				var id = curr.getAttribute( 'data-item' ),
					...
					render = inputItemWrapper.querySelector(
						'.woostify-section-render--post'
					),
					selectAllEl = inputItemWrapper.querySelector(
						'.woostify-select-all-post'
					),
					parent = inputItemWrapper.querySelector(
						'.woostify-section-select-post'
					),
			...
			if (
				'woostify-header-template-sticky' ===
				curr.getAttribute( 'name' )
			) {
				var sticky = document.querySelector(
						'input[name="woostify-header-template-sticky"]'
					).checked,
					conditionGroupSticky = curr.closest( '.condition-group' ),
					stickyChildItem = conditionGroupSticky.querySelector(
						'.child-item'
					);
				if ( sticky ) {
			...
			if (
				document.querySelectorAll( '.woostify--hf-post-name' ).length >
				0
			) {
				var inputSearch = document.querySelectorAll(
						'.woostify--hf-post-name'
					),
			...
			if ( document.querySelectorAll( '.woostify-data' ).length > 0 ) {
				var dataContainer = document.querySelectorAll(
						'.woostify-data'
					),
		...
		document.addEventListener( 'keyup', function ( e ) {
			...
			if ( btn.classList.contains( 'woostify--hf-post-name' ) ) {
				var parentItem = btn.closest( '.input-item-wrapper' ),
					postType = parentItem.querySelector( '.woostify-post-type' )
						.value,
					selectedPosts = parentItem.querySelector(
						'.woostify-post-ids'
					).value,
					keyword = btn.value;
				var request = new Request( ajaxurl, {
					method: 'POST',
					body:
						'action=woostify_pro_load_post&_ajax_nonce=' +
						nonce +
						'&post_type=' +
						postType +
						'&key=' +
				...
				fetch( request ).then( function ( res ) {
					...
					res.json().then( function ( r ) {
						...
						parentItem.getElementsByClassName(
							'woostify-data'
						)[ 0 ].innerHTML = r.data;
					} );
				} );
